...
### Obtener Vocabulario por slug (para el front)

<ApiPlayground
  method="GET"
  endpoint="/api/v1/taxonomy/vocabularies/by-slug/{slug}"
  params={{ slug: "product-categories" }}
  queryParams={{ workspace_id: "workspace-uuid (opcional)" }}
  headers={{ "X-TAXONOMY-ADAPTER": "" }}
  description="Entrega un vocabulario por slug corto con el árbol completo de términos y sus relaciones parent/child. Ideal para pre-cargar filtros en el front."
/>
...
Cada endpoint soporta cambio dinámico de repositorio via header HTTP:
...
|--------------|-------------|-----|
| `eloquent` (default) | `TermRepository` / `VocabularyRepository` | Producción |
